...
def build_phase0_lines(payload: Dict) -> List[str]:
    ...
    total_needs_review = payload.get("total_needs_review", 0)
    ...
    auto_applied_rate = (total_enriched / total_eligible * 100.0) if total_eligible > 0 else 0.0
    potential_match_rate = ((total_enriched + total_needs_review) / total_eligible * 100.0) if total_eligible > 0 else 0.0
...
def build_phase1_lines(payload: Dict) -> List[str]:
    ...
    return [
        ...
        f"Last record             : {last_record_id or '-'}",
        f"Last status             : {last_status or '-'}",
        f"Workbook/folder mismatch: {'yes' if pdfs_available_global != pdfs_in_folder else 'no'}",
    ]
...
def build_phase2_lines(payload: Dict) -> List[str]:
    ...
    if resolver_attempts:
        parts = []
        for resolver_name_item in sorted(resolver_attempts):
            attempts = resolver_attempts.get(resolver_name_item, 0)
            recovered = resolver_recovered.get(resolver_name_item, 0)
            parts.append(f"{resolver_name_item} {recovered}/{attempts}")
        resolver_summary = " | ".join(parts)

    lines = [
        "PHASE 2 - SPECIALIZED RETRY",
        f"Resolver                : {resolver_name or '-'}",
        f"Phase 2 pool            : {phase2_pool_total}",
        f"Retry total             : {retry_total}",
        f"Unassigned cases        : {unassigned_total}",
        ...
        f"Global success rate     : {success_rate_global:6.2f}%",
        f"Resolver stats          : {resolver_summary}",
        f"Elapsed                 : {format_seconds(elapsed)}",
        f"Estimated total         : {format_seconds(estimated_total)}",
        f"Estimated remaining     : {format_seconds(estimated_remaining)}",
        f"Last record             : {last_record_id or '-'}",
    ...
    if str(last_status).startswith("resolver_exception_") and last_detail:
...
def render_session_summary() -> None:
    ...
    if session.project_root:
        st.write(f"**Project root:** `{session.project_root}`")
    if session.tool_root:
        st.write(f"**Tool root:** `{session.tool_root}`")
...
def render_stored_summaries() -> None:
    ...
    with st.expander("Stored summaries", expanded=False):
        ...
        with col1:
            ...
            st.write(f"Needs review: {p0.total_needs_review}")
