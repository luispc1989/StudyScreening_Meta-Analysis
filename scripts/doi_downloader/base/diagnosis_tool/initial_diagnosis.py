...
OUTPUT_WORKBOOK_PATH = EXCEL_INPUT_DIR / f"{WORKBOOK_PATH.stem}_pdf_download_diagnostic.xlsx"
...
def print_fixed_diagnostic_panel(
    ...
    clear_terminal()
    print("=" * 78)
    print("DIAGNÓSTICO DE PDF DOWNLOAD STATUS")
    print("=" * 78)
...
def apply_stats_sheet_layout(ws) -> None:
    ...
    # Center the main ranking block
    for row_idx in range(6, ws.max_row + 1):
        for col_idx in range(1, 6):
            ws.cell(row=row_idx, column=col_idx).alignment = CENTER

    # Make total row bold if present
    for row_idx in range(1, ws.max_row + 1):
    ...
    # Center right-side block G:J
    for row_idx in range(6, ws.max_row + 1):
        for col_idx in range(7, 11):
            ws.cell(row=row_idx, column=col_idx).alignment = CENTER

    # Match the example widths
    widths = {
        "A": 8,
        "B": 34,
        "C": 10,
...
def create_stats_sheet(
    ...
    ws["A3"] = "Nota: o ranking de erros exclui o estado 'downloaded'. O campo 'website' corresponde ao host de pdf_source_url."
    ...
    style_dark_header_row(
        ws,
        5,
        ["Rank", "Erro (pdf_download_status)", "N", "% erros", "% total registos"],
        start_col=1,
    )
