...
PREFERRED_WORKBOOK_PATH = EXCEL_INPUT_DIR / "wos_workbook_03.03.2026_v2_pdf_downloaded.xlsx"
...
OUTPUT_WORKBOOK_PATH = EXCEL_INPUT_DIR / f"{WORKBOOK_PATH.stem}_pdf_error_diagnostic.xlsx"

INPUT_SHEET_NAME = "wos_full_text"
OUTPUT_SHEET_NAME = "wos_full_text"
STATS_SHEET_NAME = "pdf_error_stats"
...
# Terminal refresh tuning
PROGRESS_REFRESH_EVERY_N = 20
PROGRESS_REFRESH_EVERY_SECONDS = 1.5
...
EXCLUDED_FROM_ERROR_RANKING = {"downloaded", "", None}
...
def build_input_col_map(ws) -> dict[str, int]:
...
def validate_required_input_columns(col_map: dict[str, int]) -> None:
    missing = [c for c in REQUIRED_INPUT_COLUMNS if c not in col_map]
    if missing:
        raise ValueError(f"Missing required columns in '{INPUT_SHEET_NAME}': {missing}")


def get_input_value(ws, row_idx: int, col_map: dict[str, int], header: str):
...
def print_fixed_diagnostic_panel(
    attempted: int,
    ...
    remaining = max(total_to_process - attempted, 0)

    avg_per_article = (elapsed / attempted) if attempted > 0 else 0
    estimated_total = avg_per_article * total_to_process if total_to_process > 0 else 0
    eta_remaining = avg_per_article * remaining if attempted > 0 else 0

    clear_terminal()
    print("=" * 78)
    print("DIAGNÓSTICO DE ERROS PDF")
    print("=" * 78)
    print(f"Artigos diagnosticados : {attempted}")
    print(f"Artigos por diagnosticar: {remaining}")
    print(f"Total a diagnosticar    : {total_to_process}")
    print(f"Workers paralelos       : {MAX_WORKERS}")
    print("-" * 78)
    print(f"Tempo decorrido         : {format_seconds(elapsed)}")
    print(f"Tempo estimado total    : {format_seconds(estimated_total)}")
    print(f"Tempo restante estimado : {format_seconds(eta_remaining)}")
    print("-" * 78)
    if last_record:
        print(f"Último registo          : {last_record}")
    if last_status:
...
def format_error_tab_body(ws) -> None:
    for row_idx in range(2, ws.max_row + 1):
        for col_idx in (1, 6, 8):  # record_id, pdf_download_status, pdf_http_status
...
def create_stats_sheet(
    ...
    ws["A2"] = f"Base analisada: {base_sheet_name}"
    ws["A3"] = (
        "Nota: esta análise considera apenas registos cujo estado inicial não era "
    ...
    for row in diagnosed_rows:
        ...
        if status in EXCLUDED_FROM_ERROR_RANKING:
            continue
    ...
    style_dark_header_row(
        ws,
        5,
        ["Rank", "Erro (pdf_download_status)", "N", "% erros", "% base não-downloaded"],
        start_col=1,
    )
...
def create_error_tabs(
    ...
    for error_name in ordered_errors:
        ...
        for col_idx, header in enumerate(ERROR_TAB_COLUMNS, start=1):
...
def extract_rows_from_input(ws, col_map: dict[str, int], max_rows: Optional[int]) -> list[dict]:
    rows: list[dict] = []
    processed = 0

    for row_idx in range(2, ws.max_row + 1):
        record_id = str(get_input_value(ws, row_idx, col_map, "record_id") or "").strip()
        if not record_id:
            continue

        existing_status = str(get_input_value(ws, row_idx, col_map, "pdf_download_status") or "").strip().lower()
        if existing_status == "downloaded":
            continue

        row_values = {header: get_input_value(ws, row_idx, col_map, header) for header in REQUIRED_INPUT_COLUMNS}
        row_values["_source_row_idx"] = row_idx
        row_values["_previous_pdf_download_status"] = get_input_value(ws, row_idx, col_map, "pdf_download_status")
...
def diagnose_single_row(row_data: dict) -> dict:
    ...
    result = {
        ...
        "_previous_pdf_download_status": row_data.get("_previous_pdf_download_status", ""),
    }
    return result
...
def should_refresh_progress(
    ...
    if force:
        return True
    if attempted % PROGRESS_REFRESH_EVERY_N == 0:
        return True
    if (time.time() - last_refresh_time) >= PROGRESS_REFRESH_EVERY_SECONDS:
        return True
    return False
...
def main() -> None:
    ...
    input_wb = load_workbook(WORKBOOK_PATH, data_only=False)
    ...
    input_col_map = build_input_col_map(input_ws)
    validate_required_input_columns(input_col_map)

    rows_to_process = extract_rows_from_input(input_ws, input_col_map, MAX_ROWS_TO_PROCESS)
    ...
    print(f"Base filtrada  : apenas registos com pdf_download_status != downloaded")
    print(f"Total rows     : {total_to_process}")
    ...
    if total_to_process == 0:
        print("Não existem registos não-downloaded para diagnosticar.")

    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = [executor.submit(diagnose_single_row, row_data) for row_data in rows_to_process]
        ...
        for future in as_completed(futures):
            ...
            if should_refresh_progress(attempted, last_refresh_time):
                print_fixed_diagnostic_panel(
                    attempted=attempted,
    ...
    create_stats_sheet(
        ...
        base_sheet_name=OUTPUT_SHEET_NAME,
    )
    ...
    print(f"Ficheiro guardado: {OUTPUT_WORKBOOK_PATH}")
