...
def save_uploaded_file_to_temp(uploaded_file) -> Path:
    ...
    temp_path = temp_dir / f"streamlit_{uploaded_file.name}"
    ...
    with st.sidebar:
        ...
        if mode == "Automatic workbook":
            try:
                ...
                if PREFERRED_WORKBOOK_PATH:
                    st.caption(f"Preferred workbook configured: `{PREFERRED_WORKBOOK_PATH}`")

        else:
            uploaded_file = st.file_uploader("Upload .xlsx workbook", type=["xlsx"])
            if uploaded_file is not None:
                workbook_path = save_uploaded_file_to_temp(uploaded_file)
                st.info(f"Uploaded workbook prepared: `{workbook_path}`")
