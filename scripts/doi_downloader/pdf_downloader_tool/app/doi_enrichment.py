...
from app.models import Phase0Summary, SessionState
...
def gather_candidates(
    ...
    raw_candidates.extend(search_crossref(title))
    time.sleep(DOI_SLEEP_BETWEEN_REQUESTS)
    raw_candidates.extend(search_openalex(title))
    time.sleep(DOI_SLEEP_BETWEEN_REQUESTS)
    raw_candidates.extend(search_datacite(title))
    time.sleep(DOI_SLEEP_BETWEEN_REQUESTS)
    raw_candidates.extend(search_europe_pmc(title))
...
def row_result_to_values(row: Phase0RowResult) -> list[Any]:
    return [
        ...
        row.title,
        row.original_doi,
        row.original_doi_link,
        row.phase0_status,
        row.matched_doi,
        row.matched_doi_link,
...
def write_table_sheet(ws, rows: list[Phase0RowResult]) -> None:
    ...
    for row_result in rows:
        values = row_result_to_values(row_result)

        for col_idx, value in enumerate(values, start=1):
            cell = ws.cell(row=current_row, column=col_idx, value=value)

            if headers[col_idx - 1] in {"original_doi_link", "matched_doi_link"} and value:
                cell.hyperlink = str(value)
                cell.style = "Hyperlink"

            if headers[col_idx - 1] in {"confidence_score", "title_similarity", "year_score", "author_score", "top2_gap"}:
...
def create_phase0_report(
    ...
    write_table_sheet(enriched_ws, enriched_rows)
    ...
    write_table_sheet(unresolved_ws, unresolved_rows)
...
def run_phase0_doi_enrichment_for_session(
    ...
    for row_idx in candidate_row_indices:
        # Verificar pedido de paragem antes de cada artigo
        ...
        try:
            ...
            if best and best.title_similarity >= DOI_MIN_TITLE_SIMILARITY and best.confidence_score >= DOI_MIN_CONFIDENCE_SCORE:
                write_doi_enrichment_result(
                    ws=ws,
                    row_idx=row_idx,
                    col_map=col_map,
                ...
                phase0_status = "enriched"
                decision_reason = (
                    f"Auto-applied: title_similarity={best.title_similarity:.2f}, "
                ...
                processed_rows.append(
                    Phase0RowResult(
                        ...
                        matched_doi_link=best.doi_link,
                        ...
                        matched_journal=best.matched_journal,
                        top2_gap=top2_gap,
                ...
                if best:
                    decision_reason = (
                        f"Below thresholds: title_similarity={best.title_similarity:.2f}, "
                        f"confidence_score={best.confidence_score:.2f}"
                    )
                    processed_rows.append(
                        Phase0RowResult(
